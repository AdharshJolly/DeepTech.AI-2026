          ...
          {COMMITTEES.map((committee) => (
            <button
              ...
              className={`relative px-6 py-3 rounded-full text-sm font-bold tracking-widest uppercase transition-colors whitespace-nowrap ${
                activeTab === committee.id ? 'text-white' : 'text-ieee-gray hover:text-ieee-blue'
              }`}
            >
            ...
            {activeMembers.length > 0 ? (
              activeMembers.map((member) => (
                ...
                >
                  ...
                  <h3 className="font-heading font-bold text-lg text-ieee-black mb-1">{member.name}</h3>
                  <p className="text-sm font-semibold tracking-wide text-ieee-orange uppercase mb-3">{member.role}</p>
                  <p className="text-sm text-ieee-gray mb-6">{member.affiliation}</p>
                  
                  <div className="mt-auto flex gap-3">
                    <button className="p-2 rounded-full bg-ieee-gray/5 text-ieee-gray/40 hover:bg-ieee-blue hover:text-white transition-colors cursor-not-allowed" disabled aria-label="Mail Profile">
                      <Mail className="w-4 h-4" />
                    </button>
                    <button className="p-2 rounded-full bg-ieee-gray/5 text-ieee-gray/40 hover:bg-ieee-cyan hover:text-white transition-colors cursor-not-allowed" disabled aria-label="Website">
