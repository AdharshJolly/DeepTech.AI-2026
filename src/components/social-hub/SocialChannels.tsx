    ...
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 max-w-4xl mx-auto mt-10">
      {channels.map((ch) => (
        <a
          ...
          className={`bg-white/60 ${ch.hoverBg} backdrop-blur-xl border border-ieee-gray/10 ${ch.hoverBorder} p-6 rounded-2xl flex flex-col items-center justify-center gap-3 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg group`}
        >
          <div
            className={`w-10 h-10 ${ch.iconBg} rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform`}
          >
            {ch.icon}
          </div>
          <span className="text-sm font-bold text-ieee-black">{ch.name}</span>
        </a>
      ))}
    </div>
  );
